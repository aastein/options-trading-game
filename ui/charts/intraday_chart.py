...
class IntradayChartWidget(QChartView):

    def __init__(self):
        self._chart = QChart()
        self._chart.setTitle("Intraday 5-Second Chart")
    ...
    def update_data(self, bars: List[Tuple[datetime, float, float, float, float]]) -> None:
        if not bars:
            return
        ...
        for timestamp, open_price, high, low, close in bars:
            candle = QCandlestickSet(open_price, high, low, close)
            candle.setTimestamp(QDateTime(timestamp).toMSecsSinceEpoch())
        ...
        if bars:
            first_time = QDateTime(bars[0][0])
            last_time = QDateTime(bars[-1][0])
